...
const GroupedTeamMembers= ({employees, selectedTeam, setTeam}) => {
    ...
    function groupTeamMembers () {
        ...
        let teamA = {team:'TeamA', memebers:teamAMembers, collapsed: selectedTeam === 'TeamA'?false : true}
        teams.push(teamA);

        let teamBMembers = employees.filter((employee) => employee.teamName === 'TeamB');
        let teamB = {team:'TeamB', memebers: teamBMembers, collapsed: selectedTeam === 'TeamB'?false : true}
        teams.push(teamB);

        let teamCMembers = employees.filter((employee) => employee.teamName === 'TeamC');
        let teamC = {team:'TeamC', memebers:teamCMembers, collapsed: selectedTeam === 'TeamC'?false : true}
        teams.push(teamC);

        let teamDMembers = employees.filter((employee) => employee.teamName === 'TeamD');
        let teamD = {team:'TeamD', memebers:teamDMembers, collapsed: selectedTeam === 'TeamD'?false : true}
        teams.push(teamD);
